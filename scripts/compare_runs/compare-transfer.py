...
# Load data
df = pd.read_csv("../data/gmp-transfer/returns_space-invaders.csv", low_memory=False)
...
df = df[
    ...
    & (df["source"].isin(["Pong-v5"]))
    & (df["pruning"].isin(["dense", "gmp"]))
...
label_map = {
    ("Pong-v5", "dense", 60):   "no pruning (source: ALE/Pong-v5, 60%)",
    ("Pong-v5", "dense", 75):   "no pruning (source: ALE/Pong-v5, 75%)",
    ("Pong-v5", "gmp", 60):     "GMP 90%   (source: ALE/Pong-v5, 60%)",
    ("Pong-v5", "gmp", 75):     "GMP 90%   (source: ALE/Pong-v5, 75%)",
}
...
plt.ylabel("IQM")
plt.title("ALE/SpaceInvaders-v5 - GMP transfer (95% CI)")
plt.legend()
plt.tight_layout()

out_path = "../data/gmp-transfer/space-invaders/pong_transfer_space-invaders.png"
