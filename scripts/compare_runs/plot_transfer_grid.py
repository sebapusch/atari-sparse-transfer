...
# --- Configuration ---
DATA_DIR = "../data/lth-transfer"
OUTPUT_PATH = "../data/lth-transfer/transfer_grid.png"
...
LABELS = {
    "dense_60": "LTH60 Static",
    "dense_75": "LTH75 Static",
    "gmp_60":   "LTH60 GMP90",
    "gmp_75":   "LTH75 GMP90",
}
...
def load_and_process(target_env_slug: str, target_full_name: str, target_simple_name: str):
    ...
    # DEBUG: Check atari-lottery before filtering
    lottery_runs = df[df["project"] == "atari-lottery"]
    print(f"--- Lottery runs for {target_env_slug} (Before Seed Filter) ---")
    print(f"Count: {len(lottery_runs)}")
    print("Seeds:", lottery_runs["seed"].unique())
    ...
    # DEBUG: Check atari-lottery after filtering
    lottery_runs_after = df[df["project"] == "atari-lottery"]
    print(f"--- Lottery runs for {target_env_slug} (After Seed Filter) ---")
    print(f"Count: {len(lottery_runs_after)}")
    
    # Filter 
    valid_projects = ["lth-transfer", "atari-gmp", "atari-lottery"]
    df = df[
        (df["project"].isin(valid_projects)) &
        (df["step"] <= MAX_STEP)
    ].copy()

    # DEBUG: Check Dense Baseline Rows Before GroupBy
    dense_bl_df = df[(df["source"] == target_full_name) & (df["pruning"] == "dense")]
    print(f"DEBUG: Dense BL Rows Before GroupBy: {len(dense_bl_df)}")
    if len(dense_bl_df) > 0:
        print(f"DEBUG: Dense BL Sample: {dense_bl_df.iloc[0].to_dict()}")
    ...
    # DEBUG: Check Dense Baseline Groups
    dense_bl_groups = per_run[(per_run["source"] == target_full_name) & (per_run["pruning"] == "dense")]
    print(f"DEBUG: Dense BL Groups Count: {len(dense_bl_groups)}")
...
def main():
    ...
    # Legends
    # We create two legends? Or one combined?
    # User asked for distinction. Let's put Baseline legend on top-left of figure, Transfer on top-right?
    # Or just one row with all? 
    # "Include a first column where you display baselines"
    # Let's combine them but visually separate if possible, or just list all.
    # List: Dense, GMP 75, GMP 90 | Dense 60, Dense 75, GMP 60, GMP 75
